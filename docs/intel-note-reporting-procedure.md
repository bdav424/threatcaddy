# INTEL Intel Note Reporting Procedure

This procedure is for AI reporters and maintainers producing INTEL-style intelligence notes from ThreatCaddy investigations, daily country-hunt automation runs, or ad hoc CTI reporting.

The central rule: **an INTEL intel note is a template-fidelity task, not generic DOCX generation.** The output must look and behave like the existing INTEL report family.
...
- Any prior TeamPCP/INTEL intel note supplied in Downloads or the active run package.
...
- generated note looks like a new generic report instead of the supplied INTEL template
...
Daily ThreatCaddy country-hunt runs must follow this procedure when producing Word-ready INTEL notes. The run package should also update `Word_Render_QA_Checklist.md`, `Process_Kit.md`, and the automation update recommendation when a formatting defect or template rule changes.
