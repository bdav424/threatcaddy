...
export type IntegrationCatalogGroupId =
  | 'email'
  | 'messaging'
  | 'threat-intelligence'
...
export type IntegrationCatalogProviderStatus =
  | 'catalog-only'
  | 'design-only'
  | 'builtin-template';

export type IntegrationCatalogConfigurationStatus = 'not-configured';

export type IntegrationCatalogCapabilityStatus = 'supported' | 'design-only' | 'catalog-only';

export interface IntegrationCatalogCapability {
  id: string;
  label: string;
  status: IntegrationCatalogCapabilityStatus;
  note?: string;
}

export type IntegrationCatalogProviderNextActionKind =
  | 'route-only'
  | 'builtin-template'
  | 'future-connector';

export type IntegrationCatalogProviderNextActionSurface =
  | 'assistantcaddy-route'
  | 'integration-template'
  | 'provider-catalog';

export interface IntegrationCatalogProviderNextAction {
  kind: IntegrationCatalogProviderNextActionKind;
  label: string;
...
export interface IntegrationCatalogGroup {
  id: IntegrationCatalogGroupId;
  label: string;
  description: string;
  providers: IntegrationCatalogProvider[];
}

export interface SlackNotificationNoiseControls {
  quietHoursEnabled: boolean;
  batchChannelFollowUps: boolean;
  maxChannelFollowUpsPerHour: number;
...
export interface SlackNotificationPolicy {
  ...
  oneToOneDms: boolean;
  groupDms: boolean;
  threadRepliesAfterUserPosts: boolean;
  channelFollowUps: boolean;
  noiseControls: SlackNotificationNoiseControls;
}
