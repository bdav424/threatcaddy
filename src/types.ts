...
export interface Folder {
  ...
  /** Experimental CaddyAI memory scoped to this investigation. Defaults to disabled. */
  investigationMemoryEnabled?: boolean;
  /** Experimental isolation mode for sensitive investigations. */
  sealedCaseMemory?: boolean;
  /** Experimental privacy mode: agents should not write reusable lessons from this investigation. */
  suppressAgentLearning?: boolean;
...
export type CloudProvider = 'external-backup' | 'aws-s3' | 'azure-blob' | 'gcs';
...
export type ViewMode = 'dashboard' | 'workspace' | 'notes' | 'tasks' | 'evidence' | 'products' | 'experimental' | 'timeline' | 'whiteboard' | 'activity' | 'graph' | 'ioc-stats' | 'chat' | 'caddyassistant' | 'cademail' | 'calendarcaddy' | 'reportcaddy' | 'agent' | 'investigations' | 'reports' | 'virtualcaddy' | 'netmap' | 'journal';
...
export interface Settings {
  ...
  // --- AssistantCaddy AI (separate from ThreatCaddy AI / CaddyAI) ---
  /** When false/undefined, AssistantCaddy reuses the CaddyAI defaults above. */
  assistantLlmSeparate?: boolean;
  assistantLlmDefaultProvider?: LLMProvider;
  assistantLlmDefaultModel?: string;
  /** Optional local-endpoint overrides; fall back to the shared llmLocal* if unset. */
