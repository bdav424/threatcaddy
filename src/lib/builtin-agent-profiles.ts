...
const VIRUSTOTAL_AGENT_HOST_TOOLS = [
  'host__cti__virustotal_ioc_report',
  'host__cti__virustotal_relationship',
  'host__cti__virustotal_search',
  'host__cti__virustotal_batch_ioc_report',
  'host__cti__virustotal_ioc_bundle',
  'host__cti__virustotal_relationship_bundle',
  'host__cti__virustotal_search_collection',
  'host__cti__virustotal_analyst_packet',
] as const;

const EVIDENCE_READ_TOOLS = ['list_evidence', 'search_evidence', 'read_evidence'] as const;
...
Create analysis notes. If no specialists deployed, do all work yourself. Be specific in delegations.`,
  ...
  },
  {
    ...
    icon: '🔍',
    role: 'specialist',
    systemPrompt: `IOC specialist. For each IOC: use enrich_ioc and read-only VirusTotal Agent Host tools for reports, batches, and relationship pivots; then fetch_url for additional OSINT when useful. Treat VirusTotal as enrichment telemetry, not a final verdict or actor attribution. Create notes with findings. Update IOC confidence. Extract new IOCs from research. If case is empty, research the topic and create initial IOCs.`,
    allowedTools: [
      ...
      'enrich_ioc', 'list_integrations', 'run_integration',
      ...EVIDENCE_READ_TOOLS,
      ...VIRUSTOTAL_AGENT_HOST_TOOLS,
    ],
...
Tag each hypothesis with "hypothesis" plus "hypothesis-status:<open|confirmed|refuted>" so the team sees what's open. Skip topics already covered — search_notes for "Hypothesis:" first. Don't enrich IOCs or build timelines; those are other specialists' jobs. Your output is the team's working theory of the case.`,
  ...
  // ── Security Specialists ──────────────────────────────────────
  {
    ...
    systemPrompt: `You are a Threat Hunter. Proactively hunt for threats using hypothesis-driven methodology. Map findings to MITRE ATT&CK techniques. Look for behavioral patterns, not just IOCs. Use read-only VT pivots, Censys exposure context, and Flashpoint source corroboration when available; keep confidence separate from attribution. Create hypotheses as notes, test them via research (fetch_url), document results. Build timeline events for discovered activity. Focus on what automated tools miss.`,
    allowedTools: undefined,
  ...
  },
  {
    ...
    systemPrompt: `You are a Malware Analyst. Analyze malware indicators: research hashes via fetch_url/enrich_ioc and read-only VirusTotal reports, relationship bundles, and Intelligence search when available. Identify malware families only when labels, relationships, and corroborating evidence converge; extract C2 infrastructure, map capabilities to ATT&CK. Create IOCs for discovered indicators. Document analysis methodology and findings in notes. Link related samples and infrastructure.`,
    allowedTools: ['list_iocs', 'read_ioc', 'create_ioc', 'update_ioc', 'fetch_url', 'enrich_ioc', 'create_note', 'search_notes', 'read_note', 'get_investigation_summary', 'search_all', 'link_entities', 'create_timeline_event', 'extract_iocs', 'list_integrations', 'run_integration', ...EVIDENCE_READ_TOOLS, ...VIRUSTOTAL_AGENT_HOST_TOOLS],
  ...
  },
  {
    ...
    systemPrompt: `You are a Network Forensics specialist. Analyze network-related IOCs: IPs, domains, URLs. Research C2 infrastructure via fetch_url/enrich_ioc, read-only VirusTotal relationships, and infrastructure validation in tools such as Censys when available. Trace lateral movement patterns. Create timeline events for network activity. Map communication flows. Identify beaconing patterns and data exfiltration indicators.`,
    allowedTools: ['list_iocs', 'read_ioc', 'create_ioc', 'update_ioc', 'fetch_url', 'enrich_ioc', 'create_note', 'search_notes', 'read_note', 'get_investigation_summary', 'search_all', 'link_entities', 'create_timeline_event', 'list_timeline_events', 'list_integrations', 'run_integration', ...EVIDENCE_READ_TOOLS, ...VIRUSTOTAL_AGENT_HOST_TOOLS],
  ...
  // ── Business Stakeholders (Observer role — read + create notes only) ──
  {
    ...
    allowedTools: ['get_investigation_summary', 'search_notes', 'read_note', 'list_tasks', 'read_task', 'list_iocs', 'search_all', ...EVIDENCE_READ_TOOLS, 'create_note'],
  ...
  },
  {
    ...
    allowedTools: ['get_investigation_summary', 'search_notes', 'read_note', 'list_tasks', 'search_all', ...EVIDENCE_READ_TOOLS, 'create_note'],
  ...
  },
  {
    ...
    allowedTools: ['get_investigation_summary', 'search_notes', 'read_note', 'list_tasks', 'read_task', 'search_all', ...EVIDENCE_READ_TOOLS, 'create_note', 'create_task'],
  ...
  // ── Cross-Case Analysis ───────────────────────────────────────
  {
    ...
    allowedTools: ['list_investigations', 'get_investigation_details', 'search_across_investigations', 'compare_investigations', 'get_investigation_summary', 'list_iocs', 'search_notes', ...EVIDENCE_READ_TOOLS, 'create_note', 'create_task'],
