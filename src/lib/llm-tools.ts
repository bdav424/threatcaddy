...
import { isToolInScope, type ToolScope } from './tool-scopes';
...
export async function executeTool(
  ...
  scope: ToolScope = 'investigation',
): Promise<{ result: string; isError: boolean }> {
  const { name, input } = toolUse;

  // Hard tool-scope boundary: a tool may only run for the AI that owns its scope.
  if (!isToolInScope(name, scope)) {
    return {
      result: JSON.stringify({
        error: 'tool_out_of_scope',
        tool: name,
        callerScope: scope,
        message: `Tool "${name}" is not available to this assistant.`,
      }),
      isError: true,
    };
  }
