...
const DOCX_TEMPLATE_SANS_FONT = 'Arial';
...
export function buildTemplateBackedDocxBytes(
  ...
  const intelNoteRender = productType === 'custom-intel-note'
...
function runProperties(options: { size: string; bold?: boolean; subscript?: boolean }): string {
  return [
    '<w:rPr>',
    `<w:rFonts w:ascii="${DOCX_TEMPLATE_SANS_FONT}" w:hAnsi="${DOCX_TEMPLATE_SANS_FONT}" w:eastAsia="${DOCX_TEMPLATE_SANS_FONT}" w:cs="${DOCX_TEMPLATE_SANS_FONT}"/>`,
    options.bold ? '<w:b/>' : '',
