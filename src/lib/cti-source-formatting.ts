...
function extractFlashpointNotableTerms(rows: Record<string, unknown>[]): string[] {
  ...
  const keywordMatches = text.match(/\b(?:VENDOR|External Backup|VendorSuite|PeoplePlatform|SQL Database|cloud|database|PFAP|documents?|leak(?:ed)?|breach(?:ed)?|customer(?:s)?)\b/gi) || [];
