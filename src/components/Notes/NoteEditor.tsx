...
import { Pin, Archive, Trash2, RotateCcw, Eye, Edit3, Columns, ExternalLink, Palette, ArrowLeft, Upload, MessageSquare, Search, Lock, LockOpen, Share2, FileText, Download, AlertTriangle, Check, BookOpen, PlusCircle, X } from 'lucide-react';
...
export function NoteEditor({
  ...
  const compactPanelMode = Boolean(workspacePanelChrome?.compact);
  ...
  const pendingSaveRef = useRef<{ noteId: string; updates: Partial<Note> } | null>(null);
  ...
  const flushPendingSave = useCallback(() => {
    ...
    pendingSaveRef.current = null;
    clearPending();
    onUpdateRef.current(pending.noteId, pending.updates);
  }, []);
  ...
  }, [note.id, note.title, note.content, save, flushPendingSave]);
  ...
  const scheduleSave = useCallback((updates: Partial<Note>) => {
    ...
    pendingSaveRef.current = {
      noteId: note.id,
      updates: {
        ...(currentPending?.noteId === note.id ? currentPending.updates : {}),
        ...updates,
      },
    };
  ...
  const handleApplyAndBack = useCallback(() => {
    flushPendingSave();
    onBack?.();
  ...
  if (compactPanelMode) {
    return (
      <div className="flex h-full min-h-0 w-full flex-col overflow-hidden" data-note-compact-editor="true">
        ...
        <div className="flex shrink-0 items-center gap-2 border-b border-border-subtle/70 px-3 py-2">
          <input
            ref={titleRef}
            type="text"
            maxLength={500}
            ...
            className="min-w-0 flex-1 bg-transparent text-base font-semibold text-text-primary placeholder-text-muted focus:outline-none"
            placeholder={t('editor.titlePlaceholder')}
            readOnly={note.trashed}
            aria-label={t('editor.titleAria')}
            data-note-compact-title="true"
          />
          {onBack && (
            <>
              <button
                ...
                className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-[8px] border border-border-subtle/60 bg-bg-surface/60 text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary"
                aria-label={t('common:cancel')}
                title={t('common:cancel')}
                data-note-compact-cancel="true"
              >
                <X size={14} />
              </button>
              <button
                ...
                className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-[8px] border border-accent/25 bg-accent/10 text-accent transition-colors hover:bg-accent/16"
                aria-label={`${t('common:apply')} / ${t('editor.backToListAria')}`}
                title={`${t('common:apply')} / ${t('editor.backToList')}`}
                data-note-compact-apply-back="true"
              >
                <Check size={14} />
              </button>
            </>
        ...
        {(mergeIndicator === 'merged' || saved) && (
          <div className="pointer-events-none shrink-0 px-3 pb-2 text-[11px] text-text-muted" role="status">
            {mergeIndicator === 'merged' ? t('editor.merged') : t('editor.saved')}
          </div>
        )}
      </div>
    );
  ...
  const tlpBorderColor = getTlpBorderColor(note.clsLevel);
