...
function FloatingSourcePlaceholder({
  id,
  title,
  hidden,
  sourceHidden,
  ...
  return (
    <section
      ...
      className={cn(
        placeholderClassName,
        'w-fit max-w-[min(100%,12rem)] rounded-[10px] border border-dashed border-accent/25 bg-bg-primary/40 px-2 py-1.5 shadow-sm',
        hidden && 'hidden',
      )}
      data-workspace-panel={id}
      data-workspace-panel-state="floating-source"
      data-workspace-panel-source-compact="true"
      data-workspace-panel-source-hidden={sourceHidden ? 'true' : undefined}
    >
      <span className="inline-flex max-w-full items-center gap-1.5 align-middle">
        <span className="max-w-[7.5rem] truncate text-[10px] font-semibold uppercase text-accent">{title}</span>
        <button
          type="button"
          onClick={onDock}
          className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-[8px] border border-accent/20 bg-accent/8 text-accent transition-colors hover:bg-accent/14"
          aria-label={`${dockLabel} from source slot`}
          title={`${dockLabel} from source slot`}
        >
          <Dock size={12} aria-hidden="true" />
          <span className="sr-only">Return to workspace</span>
        </button>
      </span>
    </section>
  );
...
export function WorkspacePanel({
  ...
  const resolvedPopOutLabel = popOutLabel || `Pop out ${labelBase}`;
  const resolvedDockLabel = dockLabel || `Dock ${labelBase}`;
  const resolvedReturnLabel = resolvedDockLabel
    .replace(/^Dock\b/i, 'Return')
    .replace(/\bback into\b/i, 'to');
      ...
      >
        {mode === 'docked' ? (
          <button
            ...
            className={cn(
              'inline-flex h-7 items-center justify-center gap-1.5 rounded-full border border-accent/25 bg-accent/8 text-[10px] font-semibold text-accent transition-colors hover:bg-accent/14',
              iconOnlyPrimaryAction ? 'w-7 px-0' : 'px-2.5',
            )}
            aria-label={resolvedPopOutLabel}
            title={resolvedPopOutLabel}
            data-workspace-panel-primary-action="true"
            data-workspace-panel-primary-action-text-visible={iconOnlyPrimaryAction ? 'false' : 'true'}
          >
            <Maximize2 size={12} />
            <span className={iconOnlyPrimaryAction ? 'sr-only' : undefined}>Pop out</span>
          </button>
        ) : (
          <button
            type="button"
          ...
          >
            <Dock size={12} />
            <span className={iconOnlyPrimaryAction ? 'sr-only' : undefined}>Return</span>
          </button>
        )}
        <button
          type="button"
