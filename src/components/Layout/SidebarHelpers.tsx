...
import type { Folder, SidebarAccentStyle } from '../../types';
import { cn, formatDate } from '../../lib/utils';

function getColorChipStyle(accentColor?: string, active?: boolean) {
  if (!accentColor) return undefined;
  return {
    borderColor: accentColor,
    color: accentColor,
    backgroundColor: active ? 'var(--color-bg-active)' : 'var(--color-bg-surface)',
  } as const;
}
...
export const NavItem = React.memo(function NavItem({
  ...
  const colorChipsEnabled = accentStyle === 'color-chips';
  const hoverTitle = title ?? (draggable ? `${label}: drag into Workspace` : undefined);
  return (
    <div
      role="button"
      tabIndex={0}
      ...
      className={cn(
        ...
        colorChipsEnabled
          ? active
            ...
            : 'border border-transparent text-text-secondary hover:border-border-subtle hover:bg-bg-hover hover:text-text-primary'
          : active
    ...
    >
      ...
      {colorChipsEnabled ? (
        <span
          className={cn(
            'flex shrink-0 items-center justify-center rounded-2xl border transition-colors',
            compact ? 'h-6 w-6' : 'h-8 w-8',
          )}
          style={getColorChipStyle(accentColor, active)}
        >
          {icon}
        </span>
      ) : icon}
...
export const CollapsedIcon = React.memo(function CollapsedIcon({
  ...
  const colorChipsEnabled = accentStyle === 'color-chips';
  const hoverTitle = title ?? (draggable ? `${label}: drag into Workspace` : label);
  return (
    <div
    ...
    >
      <button
        ...
        className={cn(
          ...
          colorChipsEnabled
            ? active
              ? 'border border-border-medium bg-bg-active text-text-primary shadow-[0_10px_22px_rgba(0,0,0,0.16)]'
              : 'border border-transparent text-text-secondary hover:border-border-subtle hover:bg-bg-hover hover:text-text-primary'
            : active
              ? 'bg-bg-active text-purple'
              : 'text-text-secondary hover:bg-bg-hover hover:text-text-primary'
        )}
        style={colorChipsEnabled ? getColorChipStyle(accentColor, active) : undefined}
        aria-label={label}
        title={hoverTitle}
        data-accent-style={accentStyle}
      >
