  ...
  return (
    <section className="flex-1 flex flex-col overflow-hidden bg-bg-primary" aria-label="Activity log">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-800 shrink-0">
        ...
        <h2 className="text-sm font-medium text-gray-200">{t('title')}</h2>
          ...
          {ALL_CATEGORIES.map((cat) => {
            ...
            return (
              <button
                key={cat}
                type="button"
                aria-label={`Show ${meta.label} activity`}
      ...
      {/* Log entries */}
      <div className="flex-1 overflow-y-auto" role="list" aria-label="Activity entries">
        {filtered.length === 0 ? (
