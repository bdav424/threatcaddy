...
const BULK_KEY_MAP: Record<string, 'tiClsLevels' | 'tiIocStatuses' | 'attributionActors' | 'tiIocSubtypes' | 'tiRelationshipTypes' | 'tiDefaultClsLevel' | 'tiDefaultReportSource' | 'externalBackupLabel' | 'tiAutoExtractEnabled' | 'tiAutoExtractDebounceMs' | 'tiEnabledIOCTypes' | 'tiDefaultConfidence'> = {
...
export function ThreatIntelConfig() {
    ...
    reader.onload = () => {
      try {
        ...
        for (const [jsonKey, settingsKey] of Object.entries(BULK_KEY_MAP)) {
          ...
          } else if (typeof data[jsonKey] === 'string' && (settingsKey === 'tiDefaultClsLevel' || settingsKey === 'tiDefaultReportSource' || settingsKey === 'externalBackupLabel' || settingsKey === 'tiDefaultConfidence')) {
  ...
  const handleConfigExport = () => {
    ...
    if (settings.externalBackupLabel) config.oci_label = settings.externalBackupLabel;
