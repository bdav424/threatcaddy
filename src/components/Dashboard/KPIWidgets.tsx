...
export function KPIWidgets({ folders, allNotes, allTasks, allEvents, allIOCs, selectedKPIs, onUpdateKPIs }: KPIWidgetsProps) {
  ...
  return (
    <section className="px-6 pt-4" aria-labelledby="dashboard-key-metrics-heading">
      <div className="flex items-center justify-between mb-3">
        <h3 id="dashboard-key-metrics-heading" className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('kpi.keyMetrics')}</h3>
        <button
          onClick={() => setShowConfig(true)}
          className="p-1 rounded hover:bg-gray-700 text-gray-500 hover:text-gray-300 transition-colors"
          title="Configure KPIs"
          aria-label="Configure KPIs"
        >
          <Settings2 size={14} />
        </button>
      </div>
...
function KPIConfigModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="dashboard-kpi-config-title"
    >
      <div className="bg-gray-900 rounded-xl shadow-2xl border border-gray-700 w-full max-w-md max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h2 id="dashboard-kpi-config-title" className="text-lg font-semibold text-gray-100">Configure KPIs</h2>
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-gray-700 text-gray-400 hover:text-gray-200" aria-label="Close KPI configuration">
            <X size={20} />
          </button>
        </div>
              ...
              {draft.map((metric, i) => (
                <div key={metric} className="flex items-center gap-2 px-2 py-1.5 rounded bg-gray-800 border border-gray-700">
                  <div className="flex flex-col">
                    <button onClick={() => moveUp(i)} className="text-gray-500 hover:text-gray-300" disabled={i === 0} aria-label={`Move ${KPI_METRIC_LABELS[metric]} up`}>
                      <ChevronUp size={12} />
                    </button>
                    <button onClick={() => moveDown(i)} className="text-gray-500 hover:text-gray-300" disabled={i >= draft.length - 1} aria-label={`Move ${KPI_METRIC_LABELS[metric]} down`}>
                      <ChevronDown size={12} />
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    aria-label={`Remove ${KPI_METRIC_LABELS[metric]}`}
                  >
                    <X size={14} />
                  </button>
                </div>
