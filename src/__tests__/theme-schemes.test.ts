...
describe('applyColorScheme', () => {
  ...
  it('applies imported Odysseus schemes like any other built-in preset', () => {
    applyColorScheme('odysseus-midnight', 'dark', []);

    expect(document.documentElement.style.getPropertyValue('--color-bg-deep')).toBe('#0d1117');
    expect(document.documentElement.style.getPropertyValue('--color-accent')).toBe('#f85149');
  });
