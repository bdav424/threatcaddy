...
describe('product baselines', () => {
  ...
  it('does not ship Internal product baselines as baked-in app code', async () => {
    const { result } = await executeTool(makeToolUse('list_product_baselines'));
    const parsed = JSON.parse(result);

    expect(parsed.count).toBe(0);
    expect(parsed.baselines.some((baseline: { id: string }) => baseline.id === 'pb-custom-report')).toBe(false);
  });
  ...
  it('imports a product baseline package as an artifact-backed template', async () => {
    const imported = await importProductBaselinePackage(JSON.stringify({
      ...
      baseline: {
        ...
        productBaseline: {
          ...
          sourceDocuments: [
            { name: 'Internal_Intel_Note_Trifleck_Blockstar.docx', type: 'docx', role: 'intel-note-word-template-sample' },
          ],
          testFixtures: [
            { name: 'Internal_Intel_Note_Seedworm_South_Korea.docx', type: 'docx', role: 'daily-country-render-test-fixture' },
          ],
          layoutNotes: ['Preserve Internal Intel Note first-page header and restricted footer.'],
    ...
    expect(imported.productBaseline?.productType).toBe('custom-intel-note');
    ...
    const imported = await importProductBaselinePackage(JSON.stringify({
      ...
      baseline: {
        ...
        productBaseline: {
          ...
          testFixtures: [
            { name: 'Internal_Intel_Note_Seedworm_South_Korea.docx', type: 'docx', role: 'daily-country-render-output' },
            { name: 'South Korea custom_intel_note_context.json', type: 'json', role: 'daily-country-content-context' },
          ],
          assets: [
            { name: 'Internal_Intel_Note_Trifleck_Blockstar.docx', role: 'docx-template', mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', data: docxTemplateData },
          ],
        },
      },
