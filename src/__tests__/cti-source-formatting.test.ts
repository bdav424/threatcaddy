...
describe('cti-source-formatting', () => {
  ...
  it('supports Flashpoint query options without changing evidence ownership', () => {
    const command = parseCtiSlashCommand('/flashpoint query:"enterprise" since:now-24h size:50 site:Telegram');
    expect(command).not.toBeNull();
    const result = planCtiSourceRequests(command!, [CTI_FLASHPOINT_COMMUNITIES_TOOL]);
        ...
        items: [
          {
            ...
            message: 'PFAP alleged leak with 639,000 documents mentioning Enterprise.',
            site: 'Telegram',
            title: 'Handala Hack',
            title_id: '3686754935',
