...
describe('useLoggedActions', () => {
  ...
  describe('emptyAllTrash', () => {
    it('calls all empty trash functions and logs for each', async () => {
      ...
      mocks.evidenceItems.evidenceItems = [{ ...mocks.sampleEvidenceItem, trashed: true }];
      ...
      expect(mocks.evidenceItems.emptyTrashEvidenceItems).toHaveBeenCalled();

      // Should have logged separate empty-trash actions for every trashable entity type.
      ...
      expect(emptyTrashCalls).toHaveLength(6);
    });
  });
});
