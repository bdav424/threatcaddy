...
function makeEvidenceItem(overrides: Partial<EvidenceItem> = {}): EvidenceItem {
  return {
    ...
    content: [
      ...
      'The DOCX_TEMPLATE clipboard payload appears in the body of the report.',
    ].join('\n'),
...
describe('EvidenceView inspect preview', () => {
  ...
  it('debounces inspect search and scrolls to the settled match', async () => {
    ...
    fireEvent.change(input, { target: { value: 'DOCX_TEMPLATE' } });
    await act(async () => {
    ...
    expect(hit).toHaveTextContent('DOCX_TEMPLATE');
        ...
        items={[
          makeEvidenceItem({
            content: [
              ...
              '## Extracted Text',
              '',
              'Enterprise first hit.',
              'Enterprise second hit.',
            ].join('\n'),
          }),
        ]}
        onImportFiles={vi.fn(async () => [])}
        ...
        items={[
          makeEvidenceItem({
            content: [
              ...
              'Key Findings',
              '',
              '- Enterprise appeared in a clipboard-related note.',
            ].join('\n'),
          }),
        ]}
        onImportFiles={vi.fn(async () => [])}
  ...
  it('quarantines low-confidence PDF extraction debris in the readable preview', () => {
    ...
    expect(previewText).toContain('Enterprise and/or its');
  ...
  it('filters readable preview lines with boolean inspect queries when the switch is on', async () => {
        ...
        items={[
          makeEvidenceItem({
            content: [
              ...
              '## Extracted Text',
              '',
              'Enterprise cloud console repo',
              'Enterprise database wallet path',
    ...
    const input = screen.getByLabelText('Search inspected evidence');
    fireEvent.change(input, { target: { value: '(enterprise AND cloud) OR microsoft' } });
    await act(async () => {
      vi.advanceTimersByTime(800);
    });
    ...
    expect(previewText).toContain('Enterprise cloud console repo');
    expect(previewText).toContain('Microsoft cloud tenant address');
    expect(previewText).not.toContain('Enterprise database wallet path');
