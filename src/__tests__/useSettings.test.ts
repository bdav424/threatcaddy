...
describe('useSettings', () => {
  ...
  it('stores External Backup signed URL settings', () => {
    const { result } = renderHook(() => useSettings());

    act(() => {
      result.current.updateSettings({
        externalBackupWriteUrl: 'https://example.com/p/write/o/',
        externalBackupLabel: 'my-device',
      });
    });

    expect(result.current.settings.externalBackupWriteUrl).toBe('https://example.com/p/write/o/');
    expect(result.current.settings.externalBackupLabel).toBe('my-device');
  });

  it('migrates legacy externalBackupWriteUrl to backupDestinations', () => {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      externalBackupWriteUrl: 'https://storage.region-1.example.invalid/upload/token/container/path/',
      externalBackupLabel: 'My External Backup',
    }));
    const { result } = renderHook(() => useSettings());
    ...
    expect(dests![0].id).toBe('migrated-external-backup');
  });

  it('does not re-migrate if backupDestinations already exists', () => {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      externalBackupWriteUrl: 'https://storage.region-1.example.invalid/upload/token/container/path/',
      externalBackupLabel: 'Old',
  ...
  it('uses default label when externalBackupLabel is not set', () => {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      externalBackupWriteUrl: 'https://storage.region-1.example.invalid/upload/token/container/path/',
    }));
    const { result } = renderHook(() => useSettings());
