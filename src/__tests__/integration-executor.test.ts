...
describe('IntegrationExecutor', () => {
  ...
  describe('variable resolution', () => {
    ...
    it('builds a basic auth header from config values', async () => {
      mockFetch.mockResolvedValueOnce(
        mockJsonResponse({ data: 'ok' }),
      );
      ...
      await executor.run(
        template,
        makeInstallation({ apiId: 'legacy-id', apiSecret: 'legacy-secret' }),
        makeInput(),
        makeCallbacks(),
      );

      const [, opts] = mockFetch.mock.calls[0];
      expect(opts.headers.Authorization).toBe(`Basic ${btoa('legacy-id:legacy-secret')}`);
    });
  });
