...
// ---- External Backup Validation ----

describe('External Backup provider', () => {
  const provider = CLOUD_PROVIDERS['external-backup'];

  it('accepts a valid External Backup signed URL', () => {
    const url = 'https://storage.region-1.example.invalid/upload/abc123/container/path/';
    expect(provider.validateUrl(url)).toEqual({ valid: true });
  });

  it('rejects http:// URLs', () => {
    const url = 'http://storage.region-1.example.invalid/upload/abc123/container/path/';
    expect(provider.validateUrl(url).valid).toBe(false);
    expect(provider.validateUrl(url).error).toContain('HTTPS');
  });

  it('rejects non-External Backup hostnames', () => {
    const url = 'https://evil.com/upload/token/container/path/';
    expect(provider.validateUrl(url).valid).toBe(false);
    expect(provider.validateUrl(url).error).toContain('storage.example.invalid');
  });

  it('rejects URLs without /p/ and /o/', () => {
    const url = 'https://storage.region-1.example.invalid/n/ns/b/bucket/';
    expect(provider.validateUrl(url).valid).toBe(false);
  });

  it('builds object URL by appending path', () => {
    const base = 'https://storage.region-1.example.invalid/upload/token/container/path/';
  ...
  it('adds trailing slash to base URL if missing', () => {
    const base = 'https://storage.region-1.example.invalid/p/token/n/ns/b/bucket/o';
...
describe('detectProvider', () => {
  it('detects External Backup from hostname', () => {
    expect(detectProvider('https://storage.region-1.example.invalid/upload/token/container/path/')).toBe('external-backup');
  });
