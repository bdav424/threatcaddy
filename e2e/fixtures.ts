...
export async function goToApp(page: Page) {
  await page.addInitScript("window.__TC_CALENDARCADDY_TODAY__ = '2026-06-05T09:00:00';");
  await page.goto('/');
...
export async function navigateToView(
  ...
  const getNavButton = (label: string) =>
    viewsNav.locator(`[role="button"][aria-label="${label}"]`).first();
  const getToggleButton = (label: string) =>
    viewsNav.locator(`button[aria-label="${label}"]`).first();

  const groupToggles: Partial<Record<typeof view, string>> = {
    CaddyAI: 'Toggle Investigations menu',
    Notes: 'Toggle Investigations menu',
    Tasks: 'Toggle Investigations menu',
    Evidence: 'Toggle Investigations menu',
    Products: 'Toggle Investigations menu',
    Timeline: 'Toggle Investigations menu',
    Whiteboards: 'Toggle Investigations menu',
    IOCs: 'Toggle Investigations menu',
    Graph: 'Toggle Investigations menu',
    Activity: 'Toggle Investigations menu',
    EmailCaddy: 'Toggle AssistantCaddy menu',
    CalendarCaddy: 'Toggle AssistantCaddy menu',
  };

  const toggleLabel = groupToggles[view];
  if (toggleLabel) {
    const childButton = getNavButton(view);
    const childVisible = await childButton.isVisible().catch(() => false);
    if (!childVisible) {
      await getToggleButton(toggleLabel).click();
    }
  }

  await getNavButton(view).click();
}
